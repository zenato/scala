...
final class TraversableOps[A, Repr](val self: GenTraversableLike[A, Repr]) extends AnyVal {
  ...
  def groupConsecutiveKeys[K](f: A => K): Seq[(K, Seq[A])] = self.foldRight(List[(K, List[A])]()) {
  ...
  def simple(besides: String = "...(", postfix: String = ")", default: String = ""): String = self.headOption.map {

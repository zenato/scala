...
final class HangulString(val self: String) extends AnyVal {
  ...
  def toHangulNumber(delimiter: String = ""): String = if (!self.isDigits) self
    ...
    digits.reverseIterator.grouped(4).zipWithIndex.map { case (groupedDigits, i) =>
      val hangulDigits = groupedDigits.zipWithIndex.map { case (n, j) =>
        (if (n > 1 || j == 0 && n == 1) Numbers(n) else "") +
        (if (n > 0 && j > 0) Units(0)(j - 1) else "")
      }.reduceRight { (l, r) => r + l }

...
class RichStringSpec extends FlatSpec with Matchers {
  ...
  "#toCamelCase" should "스네이크 케이스 형식의 문자열을 카멜 케이스 형식으로 변경" in {
    "it_is_test".toCamelCase shouldBe "itIsTest"
    "it-is-test".toCamelCase shouldBe "itIsTest"
    "it is test".toCamelCase shouldBe "itIsTest"
  }
  ...
  "#checkTranscoding" should "문자열이 줘진 인코딩으로 변경이 가능한지 검사" in {
    "가힣나똫라뿧마".checkTranscoding("EUC-KR") shouldBe Set("힣", "똫", "뿧")
  }
  ...
  "#htmlCleanXSS" should "html 의 XSS(cross site scripting) 정리" in {
    ...
    val cleanedHtml =
      """<html>
        ...
        |</html>
      """.stripMargin.trim
    html.htmlCleanXSS(serializer = serializer) shouldBe cleanedHtml
  }
}
